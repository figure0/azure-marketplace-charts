...
```bash
$ helm repo add bitnami-azure https://marketplace.azurecr.io/helm/v1/repo
$ helm install bitnami-azure/node-exporter
```
...
```bash
$ helm install --name my-release bitnami-azure/node-exporter
```
...
```bash
$ helm install --name my-release --set minReadySeconds=120 bitnami-azure/node-exporter
```

Alternatively, a YAML file that specifies the values for the parameters can be provided while installing the chart. For example,

```bash
$ helm install --name my-release -f values.yaml bitnami-azure/node-exporter
```
