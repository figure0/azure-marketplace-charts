...
```bash
$ helm repo add bitnami-azure https://marketplace.azurecr.io/helm/v1/repo
$ helm install bitnami-azure/kube-state-metrics
```
...
```bash
$ helm install --name my-release bitnami-azure/kube-state-metrics
```
...
```bash
$ helm install --name my-release --set replicas=2 bitnami-azure/kube-state-metrics
```

Alternatively, a YAML file that specifies the values for the parameters can be provided while installing the chart. For example,

```bash
$ helm install --name my-release -f values.yaml bitnami-azure/kube-state-metrics
```
